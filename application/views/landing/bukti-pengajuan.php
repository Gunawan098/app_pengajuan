			...
			<div class="row">
				<div class="col-lg-12">
					<ul class="breadcrumb">
						...
						<li><a href="#">Bukti Pengajuan</a><i class="icon-angle-right"></i></li>
						<li class="active">...</li>
					</ul>
				</div>
			</div>
						    ...
						    <tbody>
						    	<?php foreach($data as $row) {?>
						    	  <?php if($row->verifikasi_kk=='sudah verifikasi' || $row->verifikasi_ektp_baru=='sudah verifikasi' || $row->verifikasi_surat_pindah_keluar=='sudah verifikasi') : ?>
									...
									<?php } ?>
							            <td style="text-align:center">
							            	<a class="btn btn-sm btn-primary" href="<?=base_url('download-bukti-pendaftaran/'.$row->id_pengajuan);?>"><i class="fa fa-download"></i> Download</a>
							            	<a class="btn btn-sm btn-info item-detail" href="javascript:void(0)" title="Detail" data="<?=$row->id_pengajuan;?>" dataa="<?=$lookup_id?>"><i class="fa fa-eye"></i> Detail</a></td>
							        </tr>
						    	  <?php endif; ?>
						    	<?php } ?>
						    </tbody>
